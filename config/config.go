...
func Load() Config {
	...
	config.UserServicePort = cast.ToString(getOrReturnDefaultValue("USER_SEVICE_PORT", ":8083"))

	config.CatalogServiceHost = cast.ToString(getOrReturnDefaultValue("CATALOG_SERVICE_HOST", "localhost"))
	config.CatalogServicePort = cast.ToString(getOrReturnDefaultValue("CATALOG_SEVICE_PORT", ":8083"))
