...
func (s *shopRepo) Create(ctx context.Context, req *us.CreateShop) (resp *us.Shop, err error) {
	...
	_, err = s.db.Exec(ctx, `
		...
		) `, id, req.Phone, req.NameUz, req.NameRu, req.NameEn, req.DescriptionUz, req.DescriptionRu, req.DescriptionEn, req.Location, req.Currency, req.PaymentTypes)
...
func (s *shopRepo) GetById(ctx context.Context, req *us.ShopPrimaryKey) (*us.Shop, error) {
	...
	err := s.db.QueryRow(ctx, `
	        ...
	        updated_at
	        FROM shop
	    WHERE id=$1`, req.Id).Scan(&resp.Id, &resp.Phone, &resp.NameUz, &resp.NameRu, &resp.NameEn, &resp.DescriptionUz, &resp.DescriptionRu, &resp.DescriptionEn, &resp.Location, &resp.Currency, &resp.PaymentTypes, &created_at, &updated_at)
...
func (s *shopRepo) GetList(ctx context.Context, req *us.GetListShopRequest) (*us.GetListShopResponse, error) {
	...
	rows, err := s.db.Query(ctx, `
		SELECT
	        ...
	        created_at,
	        updated_at
		FROM shop WHERE deleted_at is null `+filter)
	...
	for rows.Next() {
		var shop us.Shop
		count++
		err = rows.Scan(&shop.Id, &shop.Phone, &shop.NameUz,
...
func (s *shopRepo) Delete(ctx context.Context, req *us.ShopPrimaryKey) (emptypb.Empty, error) {
	_, err := s.db.Exec(ctx, `
		UPDATE category SET
		deleted_at = NOW()
		WHERE id = $1
